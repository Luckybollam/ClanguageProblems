...
# 192 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/types.h" 3 4
...
# 75 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/stat.h" 3 4
# 1 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/Availability.h" 1 3 4
...
# 412 "/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/sys/stat.h" 3 4
...
void fsize(char *name) {
    ...
    if (stat(name, &stbuf) == -1) {
        fprintf(__stderrp, "fsize: can't access %s\n", name);
        return;
    }
    ...
    printf("%s\n", ctime(&stbuf.st_mtimespec.tv_sec));
...
void dirwalk(char *dir, void (*fcn)(char *)) {
    ...
    if ((dfd = opendir(dir)) == ((void *)0)) {
        fprintf(__stderrp, "dirwalk: can't open %s\n", dir);
        return;
    }

    while ((dp = readdir(dfd)) != ((void *)0)) {
        if (strcmp(dp->name, ".") == 0 || strcmp(dp->name, "..") == 0)
            continue;

        if (strlen(dir) + strlen(dp->name) + 2 > sizeof(name)) {
            fprintf(__stderrp, "dirwalk: name %s/%s too long\n", dir, dp->name);
            continue;
        } else {
            __builtin___sprintf_chk (name, 0, __builtin_object_size (name, 2 > 1 ? 1 : 0), "%s/%s", dir, dp->name);
            (*fcn)(name);
        }
    }
...
DIR *opendir(char *dirname) {
    ...
    if ((fd = open(dirname, 0x0000, 0)) == -1
        || fstat(fd, &stbuf) == -1
        || (stbuf.st_mode & 0170000) != 0040000
        || (dp = (DIR *) malloc(sizeof(DIR))) == ((void *)0)) {
        return ((void *)0);
    }
...
Dirent *readdir(DIR *dp) {
    ...
    while (read(dp->fd, (char *) &dirbuf, sizeof(dirbuf)) == sizeof(dirbuf)) {
        ...
        d.ino = dirbuf.d_ino;
        __builtin___strncpy_chk (d.name, dirbuf.d_name, 14, __builtin_object_size (d.name, 2 > 1 ? 1 : 0));
        d.name[14] = '\0';
        return &d;
    }
